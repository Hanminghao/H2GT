...
class AttentionPooling(nn.Module):
    ...
    def forward(self, g, feat_key):
        x = g.nodes[feat_key].data['feat']
        x = self.fc(x)
        attention = self.attention(x)
        attention = torch.softmax(attention, dim=0)
        array = x * attention
        return array.mean(dim=0, keepdim=True)
    ...
    def __init__(self, in_dim, hidden_dim, out_dim, n_layers, n_heads, node_dict, dropuout, graph_pooling_type='mean'):
        ...
        for layer in range(n_layers + 1):
            ...
            if graph_pooling_type == 'sum':
                ...
                self.pools.append(AttentionPooling(in_dim, hidden_dim))
            else:
